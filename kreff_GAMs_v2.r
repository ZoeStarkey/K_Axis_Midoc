...
m1.3 <- gam(kref_b2~s(sstg_s, bs="cs"), family="nb", data=p)
m1.4 <- gam(kref_b2~s(ssh_s, bs="cs"), family="nb", data=p)
m1.5 <- gam(kref_b2~s(ssha_s, bs="cs"), family="nb", data=p)
m1.6<- gam(kref_b2~s(chl_rs_s, bs="cs"), family="nb", data=p)
m1.7<- gam(kref_b2~s(dsm_s, bs="cs"), family="nb", data=p)
m1.8<- gam(kref_b2~s(ice_s, k=3, bs="cs"), family="nb", data=p)
m1.9<- gam(kref_b2~s(dtie_s, bs="cs"), family="nb", data=p)
...
m2.7 <- gam(kref_b2~s(ssh_s, bs="cs")+
                    s(dsm_s, bs="cs"), family="nb", data=p)
...
##addtional terms don't help so just use the single term model
##m1.4 (ssh)
summary(m1.4)
...
save(m1.4, kref_pred, bc, file="~/kaxis/fish_habitat_modelling/kref_gam.Rdata")
