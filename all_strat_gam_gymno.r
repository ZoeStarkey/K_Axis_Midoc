...
##single terms
mg1a <- gam(gymno_b2~ s(bathy_s, bs="cs"), 
                    family="nb", data=p)

mg1 <- gam(gymno_b2~s(sol_pos, bs="cs", by = strat)+
...
vd$pred_epi <- predict(mg1, ndat_epi)
vd$pred_meso <- predict(mg1, ndat_meso)
vd$pred_ubathy <- predict(mg1, ndat_ubathy)

pdf("~/kaxis/fish_habitat_modelling/gymno_preds.pdf", paper="a4r", height=7.4, width=9.8)
par(mfrow=c(2,2), mar=c(3,4,3,6))

gymno_epi_pred <- setValues(env_data[[1]],as.vector(vd$pred_epi))
mn <- format(cellStats(gymno_epi_pred, mean, na.rm=T), digits=2)
tit <- paste("Gymnoscopelids: Epi ",mn  )
plot(gymno_epi_pred, col=viridis(100), zlim=c(2.8,6), axes=F, main=tit)
...
gymno_meso_pred <- setValues(env_data[[1]],as.vector(vd$pred_meso))
mn <- format(cellStats(gymno_meso_pred, mean, na.rm=T), digits=2)
tit <- paste("Gynomscopelids: Meso ",mn  )
plot(gymno_meso_pred, col=viridis(100), zlim=c(2.8,6), axes=F, main=tit)
plot(f3$finished$geometry[1:5], add=T,col="grey",lwd=3,lty=5) #SBdy
plot(bc, add=T)
points(p$lon_start, p$lat_start, cex=1.0, col="red")
degAxis(1)
...
gymno_ubathy_pred <- setValues(env_data[[1]],as.vector(vd$pred_ubathy))
mn <- format(cellStats(gymno_ubathy_pred, mean, na.rm=T), digits=2)
tit <- paste("Gymnoscopelids: Ubathy ",mn  )
plot(gymno_ubathy_pred, col=viridis(100), zlim=c(2.8,6), axes=F, main=tit)
plot(f3$finished$geometry[1:5], add=T,col="grey",lwd=3,lty=5) #SBdy
plot(bc, add=T)
points(p$lon_start, p$lat_start, cex=1.0, col="red")
degAxis(1)
...
gymno_tot_pred <- sum(gymno_epi_pred, gymno_meso_pred, gymno_ubathy_pred)
mn <- format(cellStats(gymno_tot_pred, mean, na.rm=T), digits=2)
tit <- paste("Gymnoscopelids: Total Biomass ",mn  )
...
save(mg1, gymno_epi_pred, gymno_meso_pred, gymno_ubathy_pred, gymno_tot_pred, file="~/kaxis/fish_habitat_modelling/gam_fit_gymno_all_strata.Rdata")
