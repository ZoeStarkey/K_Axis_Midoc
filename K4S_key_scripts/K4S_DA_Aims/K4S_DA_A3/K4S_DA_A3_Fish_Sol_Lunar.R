...
fish_depth_solar <- ggplot(pred_data, aes(x = altitude, y = fit)) +
  ...
  geom_line() +
  geom_point(data = km_bm_depth, aes(y = log(bm_depth_fish)), alpha = 0.5, color = "grey40", size = 2) +
  facet_wrap(~ depth, scales = "free_y", ncol = 1) +
  labs(x = "Solar Angle", y = NULL) +
  theme_minimal() +
...
fish_depth_lunar <- ggplot(pred_data, aes(x = lunar_fraction, y = fit)) +
  ...
  theme(strip.background = element_blank(),
        ...
        axis.title.x = element_text(size = 20), # Increase y-axis text size
...
output_filename <- "K4S_Plot_A3_fish_solar_lunar.png"
full_output_path <- file.path(output_directory, output_filename)
ggsave(filename = full_output_path, plot = fish_depth_sol_lunar , width =15, height =20, dpi = 500, bg = "white")
