...
oc <- inner_join(oc, dp) %>% select(-Tmin_value)
...
km$Tmin <- approxfun(oc$gmt, oc$Tmin, rule = 2)(km$start_time)
...
km$intChl <- intchl$intChl[match(km$midoc.stn, paste0("MIDOC",intchl$midoc))]
