...
# colour scheme for day/night/crepuscular
dnc<- brewer.pal(9, "Set1")[c(2:4,6)]
# add transparency
dnc <- paste0(dnc, "95")
...
read.bm.csv<- function(bm.csv){ 
  bmdat<- read.csv(bm.csv)[,-1]
  bmdat[is.na(bmdat)]<-0
  bmdat$col<- ifelse(bmdat$DNC=="N", dnc[1], 
                     ifelse(bmdat$DNC=="D", dnc[4],
                            ifelse(bmdat$DNC=="MC", dnc[2], dnc[3])))
  coordinates(bmdat) <- ~lon+lat
  bmdat
}

# total biomass for fish
stn.bm<- read.bm.csv("f.tots.csv")

# combined fish biomass split by codend
CE.bm<- read.bm.csv("f.CE.tots.csv")

# biomass of higher taxonomic groups - total
grps.bm <- read.bm.csv("stn.tax.bm.tots.csv")

# total fish biomass split by group - total
f.grps.bm <- read.bm.csv("stn.bm.tots.csv")
