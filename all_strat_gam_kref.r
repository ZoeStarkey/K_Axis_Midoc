...
plot(kref_epi_pred, col=viridis(100), zlim=c(0,7), axes=F, main=tit)
...
plot(kref_meso_pred, col=viridis(100), zlim=c(0,7), axes=F, main=tit)
plot(f3$finished$geometry[1:5], add=T,col="grey",lwd=3,lty=5) #SBdy
plot(bc, add=T)
points(p$lon_start, p$lat_start, cex=1.0, col="red")
degAxis(1)
...
plot(kref_ubathy_pred, col=viridis(100), zlim=c(0,7), axes=F, main=tit)
plot(f3$finished$geometry[1:5], add=T,col="grey",lwd=3,lty=5) #SBdy
plot(bc, add=T)
points(p$lon_start, p$lat_start, cex=1.0, col="red")
degAxis(1)
...
par(mfrow=c(3,1))
hist(prenv$bathy_b[prenv$strata=="epi"], breaks=20)
hist(prenv$bathy_b[prenv$strata=="meso"], breaks=20)
hist(prenv$bathy_b[prenv$strata=="ubathy"], breaks=20)
