...
m1.3 <- gam(gymno_b2~s(sstg_s, bs="cs"), family="nb", data=p)
m1.4 <- gam(gymno_b2~s(ssh_s, bs="cs"), family="nb", data=p)
m1.5 <- gam(gymno_b2~s(ssha_s, bs="cs"), family="nb", data=p)
m1.6<- gam(gymno_b2~s(chl_rs_s, bs="cs"), family="nb", data=p)
m1.7<- gam(gymno_b2~s(dsm_s, k=3, bs="cs"), family="nb", data=p)
m1.8<- gam(gymno_b2~s(ice_s, k=3, bs="cs"), family="nb", data=p)
m1.9<- gam(gymno_b2~s(dtie_s, bs="cs"), family="nb", data=p)
...
##two terms
m2.1 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
                    s(bathy_s, bs="cs"), family="nb", data=p)
m2.2 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
                    s(bath_g_s, bs="cs"), family="nb", data=p)
m2.3 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
                    s(sstg_s, bs="cs"), family="nb", data=p)
m2.4 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
                    s(ssh_s, bs="cs"), family="nb", data=p)
m2.5 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
                    s(ssha_s, bs="cs"), family="nb", data=p)
m2.6 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
                    s(chl_rs_s, bs="cs"), family="nb", data=p)
m2.7 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
                    s(ice_s, k=3, bs="cs"), family="nb", data=p)
m2.8 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
...
summary(m1.7)$dev.exp
summary(m2.3)$dev.exp

par(mfrow=c(2,2))
gam.check(m2.3)
plot(m2.3, scale=0, page=1)

##three terms
m3.1 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
                     s(sstg_s, bs="cs")+
                     s(bathy_s, bs="cs"), family="nb", data=p)
m3.2 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
                     s(sstg_s, bs="cs") +
                     s(bath_g_s, bs="cs"), family="nb", data=p)
m3.3 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
                     s(sstg_s, bs="cs")+
                     s(ssh_s, bs="cs"), family="nb", data=p)
m3.4 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
                     s(sstg_s, bs="cs") +
                     s(ssha_s, bs="cs"), family="nb", data=p)
m3.5 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
                     s(sstg_s, bs="cs")+
                     s(chl_rs_s, bs="cs"), family="nb", data=p)
m3.6 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
                     s(sstg_s, bs="cs")+
                     s(ice_s, k=3, bs="cs"), family="nb", data=p)
m3.7 <- gam(gymno_b2~s(dsm_s, k=3, bs="cs")+
                     s(sstg_s, bs="cs")+
                     s(dtie_s, k=3, bs="cs"), family="nb", data=p)
...
summary(m1.7)$dev.exp
summary(m2.3)$dev.exp
summary(m3.7)$dev.exp
par(mfrow=c(2,2))
gam.check(m3.7)
plot(m3.7, scale=0, page=1)
...
vd$pred <- predict(m3.7, ndat)

gymno_pred <- setValues(env_data[[1]],as.vector(vd$pred))
par(mfrow=c(1,1))
plot(gymno_pred, col=viridis(100))
plot(f3$finished$geometry[1:5], add=T,col="black",lwd=3,lty=5) #SBdy
plot(bc, add=T)
points(m1$lon_start, m1$lat_start, cex=1.5, col="red")

save(m3.7, gymno_pred, file="~/kaxis/fish_habitat_modelling/gymno_gam.Rdata")
